...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // 팟캐스트 오디오 파일 다운로드 라우트
  app.get('/api/podcast/download/:filename', isAuthenticated, (req, res) => {
    try {
      const filename = req.params.filename;
      const filePath = path.join(process.cwd(), 'uploads', filename);
      ...
      // 파일 이름에서 확장자 제거하고 의미있는 이름으로 변경
      const originalName = filename.replace(/^\d+-/, '').replace(/\.wav$/, '');
      const downloadName = `podcast-${originalName || 'audio'}.wav`;
  ...
  // Test audio generation route
  app.post('/api/test-audio-generation', isAuthenticated, async (req: any, res) => {
    try {
      const { contentId, language } = req.body;
      
      if (!contentId || !language) {
        return res.status(400).json({ message: 'Content ID and language are required' });
      }
      
      // Get the content to test audio generation
      ...
      console.log(`Testing audio generation for content: ${content.title}`);
      
      // Create test script based on content type
      let testScript = '';
      if (content.contentType === 'summary') {
        testScript = `${content.title}\n\n주요 내용: ${content.content.mainContent || ''}`;
      } else if (content.contentType === 'quiz') {
        testScript = `${content.title}\n\n퀴즈 문제 예시: ${content.content.questions?.[0]?.question || ''}`;
      } else if (content.contentType === 'study_guide') {
        testScript = `${content.title}\n\n학습 목표: ${content.content.learningObjectives?.[0] || ''}`;
      } else {
        testScript = `${content.title}\n\n테스트용 오디오 생성입니다.`;
      }
      
      // Limit script length for testing
      if (testScript.length > 200) {
        testScript = testScript.substring(0, 200) + '...';
      }
      
      console.log(`Test script length: ${testScript.length} characters`);
      
      // Try to generate audio using current available models
      ...
      for (const modelName of modelsToTry) {
        try {
          console.log(`Testing audio generation with model: ${modelName}`);
          
          const response = await ai.models.generateContent({
            model: modelName,
            contents: [{
              role: "user",
              parts: [{ text: `다음 텍스트를 자연스러운 한국어로 읽어주세요: ${testScript}` }]
            }],
            config: {
          ...
          console.log(`✓ Audio generation successful with model: ${modelName}`);
          audioGenerated = true;
          
          res.json({
            success: true,
            message: `오디오 생성 테스트 성공! 모델: ${modelName}`,
            model: modelName,
          ...
          // Try TEXT modality if AUDIO fails
          try {
            console.log(`Trying TEXT modality with model: ${modelName}`);
            ...
            console.log(`✓ TEXT generation successful with model: ${modelName}`);
            
          } catch (textError: any) {
            console.log(`Model ${modelName} TEXT also failed:`, textError.message || textError);
            continue;
          }
        }
      }
      ...
      if (!audioGenerated) {
        ...
        console.log(`All models failed for audio generation. Last error: ${errorMessage}`);
        
        res.json({
          success: false,
          message: `모든 모델에서 오디오 생성 실패. 오류: ${errorMessage}`,
          error: errorMessage
        });
      }
      
    } catch (error) {
      console.error('Error in audio generation test:', error);
      res.status(500).json({ 
        success: false,
  ...
  app.get('/api/drive/files', isAuthenticated, async (req: any, res) => {
    try {
      ...
      } else if (error.action === 'API_NOT_ENABLED') {
        res.status(403).json({ 
      ...
      } else if (error.action === 'REAUTHENTICATE') {
        res.status(401).json({ 
          message: 'Google Drive 인증이 만료되었습니다. 다시 로그인해주세요.',
          action: error.action
        });
