...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Create Google Docs from content
  app.post('/api/content/:id/google-docs', isAuthenticated, async (req: any, res) => {
    try {
      const { id } = req.params;
      const userId = req.user?.id;
      ...
      const { createDriveService } = await import('./services/googleDrive');
      ...
      if (item.contentType === 'summary') {
        ...
        contentText = `# ${itemContent.title}\n\n## 학습 목표\n${itemContent.learningObjectives?.join('\n- ') || ''}\n\n## 주요 개념\n`;
        if (itemContent.keyConcepts) {
          itemContent.keyConcepts.forEach((concept: any) => {
            contentText += `**${concept.term}**: ${concept.definition}\n\n`;
          });
        }
        contentText += `\n## 학습 질문\n${itemContent.studyQuestions?.join('\n\n') || ''}`;
      } else if (item.contentType === 'podcast') {
        contentText = `# ${itemContent.title}\n\n## 설명\n${itemContent.description}\n\n## 스크립트\n${itemContent.script || ''}`;
      }

      // Create Google Doc
      const { docId, docUrl } = await driveService.createGoogleDoc(
      ...
      res.json({
        ...
        docId,
        docUrl,
        message: 'Google Docs 문서가 성공적으로 생성되었습니다.'
      });
